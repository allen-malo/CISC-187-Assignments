...
For this example, we are starting with an empty stack that can contain 6 elements. The important thing to note here is that the stack data structure is LIFO (Last in, First out), so whatever element we add first is going to be the one we access last. A better way to picture this is like a stack of plates. In order to get to the bottom plate (without breaking the others), you have to remove the plate on top first to get closer and closer to the bottom plate.
